...
MVPTree* mvptree_alloc(MVPTree *tree, CmpFunc distance,unsigned int bf,unsigned int p,\
    ...
    retTree->datatype     = static_cast<MVPDataType>(0);
    retTree->node         = NULL;
    retTree->fd           = 0;
    retTree->k            = 0;
    retTree->size         = 0;
...
#else
    retTree->pgsize       = 4096; // FIXME: bad value
#endif
    retTree->extDataPtr   = NULL;
    return retTree;
}
...
static Node* create_internal(unsigned int bf){
    ...
    node->internal.child_nodes = static_cast<void**>(calloc(bf*bf,sizeof(Node*)));
...
static void free_node(Node *node){
    if (node) { 
        if (node->leaf.type == LEAF_NODE) {
            // FIXME: add check for NULL-pointers
            if(node->leaf.points) {
//                try {
...
static void _mvptree_clear(MVPTree *tree, Node *node, MVPFreeFunc free_func, int lvl){
    ...
    if (node->internal.type == INTERNAL_NODE){
        ...
        for (i = 0;i < fanout;i++){
        _mvptree_clear(tree, static_cast<Node*>(node->internal.child_nodes[i]), free_func, lvl+1);
        }
...
static int select_vantage_points(MVPDP **points, unsigned int nb,int *sv1_pos, int *sv2_pos,\
    ...
    *sv2_pos = -1;
    ...
    for (i = 0; i < nb; i++){
        for (j = i+1; j < nb; j++){
            d = dist(points[i], points[j], extDataPtr);
            if (is_nan(d) || d < 0.0f){
                return -2;
            }
            if (d > max_dist){
...
static int find_splits(MVPDP **points,unsigned int nb,MVPDP *vp,MVPTree *tree,\
    ...
    int i, j, error = 0;
    for (int i = 0;i < nb; i++){
        dist[i] = distfunc(points[i], vp, tree->extDataPtr);
...
static MVPDP*** sort_points(MVPDP **points, unsigned int nbpoints, int sv1_pos, int sv2_pos,\
    ...
    *counts = (int*)calloc(bf,sizeof(int));
    ...
    for (i=0;i<nbpoints;i++){
        ...
        float d = distfunc(vp, points[i], tree->extDataPtr);
        if (is_nan(d) || d < 0.0f){
            free(counts);
            free(bins);
            return NULL;
...
static int find_distance_range_for_vp(MVPDP **points, unsigned int nbpoints, MVPDP *vp,\
    ...
    int i,j, error = 0;
    for (i = 0; i < nbpoints; i++){
        float d = func(vp, points[i], tree->extDataPtr);
        if (is_nan(d) || d < 0.0f){
            return -2;
        }
...
static Node* _mvptree_add(MVPTree *tree, Node *node, MVPDP **points, unsigned int nbpoints,\
    ...
    if (new_node == NULL){ /* create new node */
        ...
        if (nbpoints <= tree->leafcap + 2){
            ...
            if (select_vantage_points(points, nbpoints, &sv1_pos, &sv2_pos, tree->dist, tree->extDataPtr) < 0){
            ...
            for (i=0;i<nbpoints;i++){
                if (i == sv1_pos || i == sv2_pos) continue;
                new_node->leaf.d1[count] = dist_fnc(points[i], new_node->leaf.sv1, tree->extDataPtr);
                new_node->leaf.d2[count] = dist_fnc(points[i], new_node->leaf.sv2, tree->extDataPtr);
        ...
        if (new_node->leaf.type == LEAF_NODE){
            ...
            if (new_node->leaf.nbpoints + nbpoints <= tree->leafcap){
                ...
                for (; pos < nbpoints;pos++){
                    new_node->leaf.d1[count] = tree->dist(points[pos], new_node->leaf.sv1, tree->extDataPtr);
                    new_node->leaf.d2[count] = tree->dist(points[pos], new_node->leaf.sv2, tree->extDataPtr);
            ...
            for (i=0 ;i < tree->branchfactor; i++ ){
                ...
                for (j=0;j<tree->branchfactor;j++){
                    ...
                    child = _mvptree_add(tree,\
                                  static_cast<Node*>(new_node->internal.child_nodes[i*tree->branchfactor + j]),\
...
MVPError _mvptree_retrieve(MVPTree *tree,Node *node,MVPDP *target, float radius, MVPDP** results,\
    ...
    if (node->leaf.type == LEAF_NODE){
        d1 = distance(target, node->leaf.sv1, tree->extDataPtr);
        ...
        if (tree->node->leaf.sv2){
            d2 = distance(target, node->leaf.sv2, tree->extDataPtr);
        ...
        */
                ...
                if (d1 - radius <= node->leaf.d1[i] && d1 + radius >= node->leaf.d1[i]){
                    if (d2 - radius <= node->leaf.d2[i] && d2 + radius >= node->leaf.d2[i]){
                        ...
                        if (!skip){
                            ...
                            float d = distance(target, node->leaf.points[i], tree->extDataPtr);
                            if (is_nan(d) || d < 0.0){
                                return MVP_BADDISTVAL;
                            }
                            if (d <= radius){
        ...
        d1 = distance(target, node->internal.sv1, tree->extDataPtr);
        if (is_nan(d1) || d1 < 0.0f){
            return MVP_BADDISTVAL;
        }
        if (d1 <= radius){
        ...
        if (lvl < tree->pathlength) target->path[lvl] = d1;
        d2 = distance(target, node->internal.sv2, tree->extDataPtr);
        if (is_nan(d2) || d2 < 0.0f){
            return MVP_BADDISTVAL;
        }
        if (d2 <= radius){
        ...
        for (i=0;i<lengthM1;i++){
            ...
            err = _mvptree_retrieve(tree,
                                    static_cast<Node*>(node->internal.child_nodes[i*bf+j]),target,\
            ...
            err = _mvptree_retrieve(tree,static_cast<Node*>(node->internal.child_nodes[i*bf+lengthM1]),\
        ...
        if (d1 + radius >= node->internal.M1[lengthM1-1]){
            ...
            err = _mvptree_retrieve(tree,static_cast<Node*>(node->internal.child_nodes[bf*lengthM1+j]),\
                                            target, radius, results, nbresults, lvl+2);
                    if (err != MVP_SUCCESS) return err;
                }
            }
        ...
        err = _mvptree_retrieve(tree,static_cast<Node*>(node->internal.child_nodes[bf*lengthM1+lengthM1]),\
                                         target, radius, results, nbresults, lvl+2);
                if (err != MVP_SUCCESS) return err;
            }
        }
...
static off_t write_datapoint(MVPDP *dp, MVPTree *tree){
    ...
    pos += sizeof(uint32_t);
    //FIXME:  write external idx
    int tmpIdx  = dp->idx;
    memcpy(&buf[pos]  , &tmpIdx, sizeof(int));
    pos += sizeof(int);
    //
...
static off_t _mvptree_write(MVPTree *tree,Node *node,MVPError *error,int lvl) {
    ...
    if (node->leaf.type == LEAF_NODE){
        ...
        for (i=0;i<fanout;i++){
            ...
            off_t offset = _mvptree_write(tree, static_cast<Node*>(node->internal.child_nodes[i]), error, lvl+2);
...
static MVPDP* read_datapoint(MVPTree *tree) {
    ...
    tree->pos += sizeof(uint32_t);
    //FIXME: read external-idx
    int tmpIdx;
    memcpy(&tmpIdx, &tree->buf[tree->pos], sizeof(int));
    tree->pos   += sizeof(int);
    dp->idx     = tmpIdx;
    //
...
static MVPError _mvptree_print(FILE *stream, MVPTree *tree, Node *node, int lvl){
    ...
    if (next_node){
        if (next_node->leaf.type == LEAF_NODE){
            ...
            fprintf(stream,"\n");
            for (i=0;i<fanout;i++){
        error = _mvptree_print(stream, tree, static_cast<Node*>(node->internal.child_nodes[i]), lvl+2);
                if (error != MVP_SUCCESS) break;
            }
        } else {
            error = MVP_UNRECOGNIZED;
